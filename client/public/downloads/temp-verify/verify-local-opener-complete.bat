...
) else ( FAIL: Non sei amministratore
...
if exist "%~dp0cruscotto-local-opener-setup.exe" (
    ...
    set /a TESTS_PASSED+=1
) else ( FAIL: File exe non trovato
...
) else ( FAIL: Task LocalOpenerAuto non trovato
...
if %errorLevel% equ 0 (
    ...
    set /a TESTS_PASSED+=1
) else ( FAIL: Servizio non risponde
...
) else ( FAIL: Impossibile contattare API rilevamento
    set /a TESTS_FAILED+=1
)
...
) else ( FAIL: API apertura file non risponde
    set /a TESTS_FAILED+=1
)
...
if %errorLevel% equ 0 (
    ...
    set /a TESTS_PASSED+=1
) else ( FAIL: Porta 17654 non in ascolto
...
if %TESTS_FAILED% equ 0 (
    ...
    color 0C PROBLEMI RILEVATI! Diversi test falliti.
