...
Write-Host "DISINSTALLAZIONE COMPLETA LOCAL OPENER" -ForegroundColor Red
...
try {
    ...
    Write-Host "Errore nell'arresto del servizio" -ForegroundColor Yellow
}

Write-Host ""
Write-Host "2. Rimozione del servizio Windows..." -ForegroundColor Cyan
try {
    $service = Get-Service -Name $ServiceName -ErrorAction SilentlyContinue
    if ($service) {
        if (Test-Path $NssmPath) {
        ...
        Write-Host "Servizio già rimosso" -ForegroundColor Gray
    }
} catch {
    Write-Host "Errore nella rimozione del servizio" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "Errore rimozione firewall" -ForegroundColor Yellow
}
...
try {
    ...
    if (Test-Path $LogDir) {
        ...
        Write-Host "Cartella log non trovata" -ForegroundColor Gray
    }
} catch {
    Write-Host "Errore pulizia log" -ForegroundColor Yellow
}
...
try {
    ...
    if ($LocalOpenerProcesses) {
        ...
        Write-Host "Nessun processo Local Opener in esecuzione" -ForegroundColor Gray
    }
} catch {
    Write-Host "Errore terminazione processi" -ForegroundColor Yellow
}
...
if ($PortInUse) {
    Write-Host "Porta 17654 ancora in uso - potrebbe essere necessario riavviare" -ForegroundColor Yellow
} else {
...
Write-Host ""
Write-Host "NUOVE FUNZIONALITA NELLA VERSIONE AGGIORNATA:" -ForegroundColor Green
...
Write-Host "Diagnostica avanzata con auto-riparazione" -ForegroundColor White
