...
function Add-DetectedPath {
    ...
    if (Test-Path $Path -PathType Container) {
        try {
            ...
            if (-not $Silent) {
                Write-Host "⚠️ Non accessibile: $Path (da $Source)" -ForegroundColor Yellow
            }
        }
    } else {
        $DetectedPaths += @{
        ...
        if (-not $Silent -and $VerboseOutput) {
            Write-Host "❌ Non valido: $Path (da $Source)" -ForegroundColor Red
        }
    }
}
...
# 1. RICERCA REGISTRO WINDOWS - Google Drive Desktop (più efficiente)
...
try {
    ...
    if ($GoogleDriveRegistry) {
        if (-not $Silent) { Write-Host "   ✅ Google Drive Desktop rilevato nel registro" -ForegroundColor Green }
    ...
    if (-not $Silent) {
        Write-Host "   ⚠️ Errore accesso registro: $($_.Exception.Message)" -ForegroundColor Yellow
    }
}
...
try {
    ...
    if (-not $Silent) {
        Write-Host "   ⚠️ Errore scansione utenti: $($_.Exception.Message)" -ForegroundColor Yellow
    }
}
...
# Test solo le lettere più comuni per Google Drive
...
if (-not $Silent) {
    ...
    if ($ValidPaths.Count -gt 0) {
        ...
        Write-Host "   • Controlla se Google Drive è montato su lettere G:\, H:\, ecc." -ForegroundColor White
...
if ($ConfigureService -and $ValidPaths.Count -gt 0) {
    if (-not $Silent) {
        Write-Host ""
        Write-Host "⚙️ Configurazione automatica Local Opener..." -ForegroundColor Cyan
    }
    ...
    try {
        ...
        if (-not $Silent) {
            ...
            Write-Host "ℹ️ Configura manualmente dal pannello web: http://127.0.0.1:17654" -ForegroundColor Cyan
        }
    }
}
